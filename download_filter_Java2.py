...
def mvn_compile(i,f_dir,dep,log,fh):
    ...
    if ret>0:
        log.error("Error in original mvn compile under the current directory: ",str(i))
        return 1
    return 0
...
def mvn_test(i,f_dir,dep,log,fh):
    ...
    if ret>0:
        log.error("Error in original mvn test under the current directory: ",str(i))
        return 1
    return 0
...
def mvn_process(i,log,fh):
    ...
    if err_xml!=None or int(output_xml)==0: ##error or no found code
        log.error("stop the process and delete the directory because no maven pom.xml or grep error")
    ...
    with open(proj_list) as rfile:
	    ...
	    print("i: "+str(i))
            log,fh = createLog(log_path+str(i)+".log")
       	    line=line.strip()
            download(i,line,log,fh)
	    print("------------------------")
            output_xml,pom_depth,pom_need,pom_error,test_error=mvn_process(i,log,fh)
	    print("------------------------")
...
if __name__== '__main__':
	...
	print begin, end
    	os.chdir(ws)
	download_instrument(int(begin),int(end))
