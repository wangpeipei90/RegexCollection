...
import sys
import logging
from edit_pom_py3 import agent_pom
import signal
...
timeout_log="/home/peipei/RepoReaper/RegexCollection/timeout.log"
#proj_timeout= open(timeout_log,'a') ##'w' for only writing (an existing file with the same name will be erased), and 'a' opens the file for appending

time=7200 ##3600 secs == 1 hour
#time=60 ##5*60 secs == 5 min
...
def sortPom(log,fh):
    ...
    #print("p_pom_sort.pid: ",p_pom_sort.pid)
#    p_pom2.stdout.close()
    cleanUp(p_pom2)
    p_pom2.wait()
...
def mvn_compile(i,f_dir,dep,log,fh):
    ...
    #print("p_mvn_compile.pid: ",p_mvn_compile.pid)
...
def mvn_test(i,f_dir,dep,log,fh,isFirst):
    print("now run mvn test")
    log.info("now run mvn test")
    #p_mvn_test=subprocess.Popen(["mvn","test"],stdout=subprocess.PIPE, preexec_fn=os.setpgrp)
    p_mvn_test=subprocess.Popen(["mvn","test"],stdout=subprocess.PIPE)
    #print "mvn test sys stdout"
    #print("p_mvn_test.pid: ",p_mvn_test.pid)
    try:
        print ("timeout: ",time)
        output,err=p_mvn_test.communicate(timeout=time)
...
#        p_mvn_test.stdout.close()
        cleanUp(p_mvn_test)
#        os.killpg(p_mvn_test.pid, signal.SIGTERM)
        # p_mvn_test.terminate()
        p_mvn_test.kill()
        proj_timeout= open(timeout_log,'a')
        if isFirst:
            proj_timeout.write("Original mvn test Exception time out: "+str(i))
        else:
            proj_timeout.write("Instrumented mvn test"+" Exception time out: "+str(i))
        proj_timeout.write("\n")
        proj_timeout.close()
...
def mvn_process(i,log,fh):
    ...
    pom_error=0
    test_error=0
    min_depth=output_sorts[0].decode('utf-8')
    f_dir=output_sorts[1].decode('utf-8')
    ...
    if f_dir==".":
        ...
        if comp_error:
            ...
            t_error=mvn_test(i,f_dir,min_depth,log,fh,True)
            if t_error:
                test_error+=1
            else:
                ...
                t_error=mvn_test(i,f_dir,min_depth,log,fh,False)
    else:
        print("pom not in root project folder")
        dirs=0
        while dirs<int(output_xml): ##even is depth and odd is dirpath
            ...
            if cur_depth==min_depth:
                pom_need+=1
                cur_dir=output_sorts[2*dirs+1].decode('utf-8')
                cur_cwd=os.getcwd()
                os.chdir(cur_dir)
                ...
                if comp_error:
                    ...
                    t_error=mvn_test(i,cur_dir,cur_depth,log,fh,True)
                    if t_error:
                        test_error+=1
                    else:
                        log.info("change pom.xml")
